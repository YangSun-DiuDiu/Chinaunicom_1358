...
> **For agentic workers:** 使用 superpowers:subagent-driven-development 或 superpowers:executing-plans 逐步实施。步骤使用 checkbox 语法跟踪。

**Goal:** 将首页重构为统一4模块驾驶舱（IoT设备+访客+会议+公寓），深色科技蓝主题

**Architecture:** 后端新增2个 Dashboard API → 前端新增 API 函数 → 重写 `views/index.vue` 单文件

**Tech Stack:** SpringBoot2 + MyBatis + Vue2 + ECharts + vue-count-to

## Global Constraints

- 不改动若依框架底层源码
- 所有 API 遵循 `AjaxResult` 统一返回
- 前端使用 `request` 统一封装
- 配色严格遵循 `#0a1a2e` 底色 + `#00d4ff` 强调色
- 所有数据来自真实 API，禁止硬编码

---

### Task 1: 扩展 IDashboardService 接口

**Files:**
- Modify: `ruoyi-system/src/main/java/com/ruoyi/system/service/IDashboardService.java:25`

**Interfaces:**
- Produces: `getMeetingDashboard()`, `getApartmentDashboard()` 返回 `Map<String, Object>`

- [ ] **Step 1: 添加两个新方法签名**

```java
// 在现有 getVisitorDashboard() 后面添加:
...
```bash
git add ruoyi-system/src/main/java/com/ruoyi/system/service/IDashboardService.java
git commit -m "feat: IDashboardService新增meeting/apartment仪表盘方法签名"
```

---

### Task 2: 实现 DashboardServiceImpl 新增方法

**Files:**
- Modify: `ruoyi-system/src/main/java/com/ruoyi/system/service/impl/DashboardServiceImpl.java:1-185`

**Interfaces:**
- Consumes: `MeetingBookingMapper.selectMeetingBookingList()`, `MeetingRoomMapper.selectMeetingRoomList()`, `RoomInfoMapper.selectRoomCardList()`
- Produces: `getMeetingDashboard()` → `Map<String, Object>`, `getApartmentDashboard()` → `Map<String, Object>`

- [ ] **Step 1: 添加 Mapper 注入**

在现有 `@Autowired private VisitorLogMapper visitorLogMapper;` 后添加：
...
- [ ] **Step 2: 添加 getMeetingDashboard 方法**

在 `getVisitorDashboard()` 方法后添加：
...
- [ ] **Step 3: 添加 getApartmentDashboard 方法**
...
```java
    ...
    public Map<String, Object> getApartmentDashboard()
    {
        ...
        // 按状态分布统计（GREEN=空置, CYAN=部分入住, BLUE=满房, GRAY=维修）
...
```bash
git add ruoyi-system/src/main/java/com/ruoyi/system/service/impl/DashboardServiceImpl.java
git commit -m "feat: DashboardServiceImpl实现会议和公寓仪表盘方法"
```

---

### Task 3: 添加 DashboardController 新端点

**Files:**
- Modify: `ruoyi-admin/src/main/java/com/ruoyi/web/controller/dashboard/DashboardController.java:45`

**Interfaces:**
- Produces: `GET /dashboard/meeting`, `GET /dashboard/apartment`
...
```bash
git add ruoyi-admin/src/main/java/com/ruoyi/web/controller/dashboard/DashboardController.java
git commit -m "feat: DashboardController新增meeting/apartment端点"
```

---

### Task 4: 前端 API — 新增会议和公寓仪表盘函数

**Files:**
- Modify: `ruoyi-ui/src/api/dashboard/dashboard.js`
...
export function getDeviceDashboard() {
  return request({ url: '/dashboard/device', method: 'get' })
}

export function getVisitorDashboard() {
  return request({ url: '/dashboard/visitor', method: 'get' })
}
```

- [ ] **Step 2: 添加两个新 API 函数**

在文件末尾 `getVisitorDashboard` 后添加：
...
```bash
git add ruoyi-ui/src/api/dashboard/dashboard.js
git commit -m "feat: 前端API新增getMeetingDashboard/getApartmentDashboard"
```

---

### Task 5: 重写首页驾驶舱 views/index.vue

**Files:**
- Rewrite: `ruoyi-ui/src/views/index.vue`

**Interfaces:**
- Consumes: `getDeviceDashboard`, `getVisitorDashboard`, `getMeetingDashboard`, `getApartmentDashboard` from `@/api/dashboard/dashboard`
- Produces: 驾驶舱首页，无外部依赖

- [ ] **Step 1: 备份原文件**

```bash
cp ruoyi-ui/src/views/index.vue ruoyi-ui/src/views/index_old.vue
```
...
```bash
git add ruoyi-ui/src/views/index.vue ruoyi-ui/src/views/index_old.vue
git commit -m "feat: 重写首页为4模块统一驾驶舱(深蓝科技主题)"
```
...
ssh root@1.94.26.126 'cd /root/zhihuiyuanqu20260625 && bash stop-server.sh && sleep 2 && bash start-server.sh'
```
...
```bash
# 等待服务启动后
curl -s http://localhost:8090/dashboard/meeting | python3 -m json.tool
curl -s http://localhost:8090/dashboard/apartment | python3 -m json.tool
```
...
如需更新前端：
```bash
cd ruoyi-ui && npm run build:prod && scp -r dist/* root@1.94.26.126:/root/zhihuiyuanqu20260625/dist/
```

- [ ] **Step 5: 浏览器验证**

打开 `http://1.94.26.126` 登录，检查首页：
- [ ] 深蓝底色正确
- [ ] 4 个 KPI 数字真实（非硬编码）
- [ ] 设备饼图/访客柱状图/公寓环形图正常
- [ ] 今日会议列表显示
...
```bash
git add -A && git commit -m "chore: 驾驶舱重构部署验证完成"
