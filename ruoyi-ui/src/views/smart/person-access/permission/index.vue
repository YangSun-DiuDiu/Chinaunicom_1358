  ...
  <div class="app-container">
    ...
    <el-form :model="query" ref="queryForm" size="small" :inline="true" label-width="68px">
      ...
      <el-form-item label="授权类型" prop="target_type">
        <el-select v-model="query.target_type" placeholder="全部" clearable style="width:120px">
    ...
    <!-- 新增授权弹窗 -->
    <el-dialog title="新增人员通行权限" :visible.sync="open" width="700px" append-to-body :close-on-click-modal="false">
      <el-form ref="form" :model="form" :rules="rules" label-width="90px" size="small">
        ...
        <el-form-item v-if="form.target_type==='DEPT' && form.target_name" label="已选部门">
          ...
          <span style="color:#909399;font-size:12px;margin-left:8px">该部门下所有人员自动获得通行权限</span>
        </el-form-item>
...
export default {
  name: 'PersonAccessPermission',
  data() {
    return {
      loading: false, total: 0, list: [], open: false, submitting: false, dateRange: [],
      deptList: [], userList: [], deviceList: [], deviceMap: {},
      query: { pageNum: 1, pageSize: 10, target_name: undefined, target_type: undefined, status: undefined },
      form: { target_type: 'DEPT', target_id: null, target_name: '', selectedDevices: [], remark: '' },
      rules: {
  ...
  methods: {
    ...
    getList() {
      ...
      request({ url: '/smart/access/permission/person/list', method: 'get', params }).then(r => {
        this.list = r.rows || []; this.total = r.total || 0; this.loading = false
      }).catch(() => { this.loading = false })
    },
    resetQuery() {
      this.query = { pageNum: 1, pageSize: 10, target_name: undefined, target_type: undefined, status: undefined }
      this.getList()
    },
    loadDeptAndUser() {
    ...
    loadDeviceList() {
      request({ url: '/smart/access/person-device/list', params: { pageSize: 200 } }).then(r => {
        const rows = r.rows || []
        this.deviceMap = {}
    ...
    openAdd() { this.form = { target_type: 'DEPT', target_id: null, target_name: '', selectedDevices: [], remark: '' }; this.dateRange = []; this.open = true; this.$nextTick(() => { this.$refs.form && this.$refs.form.clearValidate() }) },
    submit() {
      this.$refs.form.validate(v => {
        if (!v) return
        this.submitting = true
        request({ url: '/smart/access/permission/person', method: 'post', data: {
          target_type: this.form.target_type, target_id: this.form.target_id,
          target_name: this.form.target_name, device_ids: (this.form.selectedDevices || []).join(','),
    ...
    handleDel(row) {
      this.$confirm('确认取消对【' + row.target_name + '】的通行授权吗？', '提示', { type: 'warning' }).then(() => {
        request({ url: '/smart/access/permission/person/' + row.perm_id, method: 'delete' }).then(() => { this.$message.success('删除成功'); this.getList() })
      }).catch(() => {})
    }
  }
}
